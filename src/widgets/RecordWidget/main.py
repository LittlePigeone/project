...
from widgets.common.add_name import AddName
...
class AudioRecorder(QWidget):
    def __init__(self, sounds_dir):
        ...
        self.add_name_widet = AddName()
        self.add_name_widet.name_saved.connect(self.handle_name_saved)
    ...
    def stop_recording(self):
        if self.recording:
            ...
            self.stream.stop()
            self.stream.close()
            self.add_name_widet.show()
            self.status_label.setText("Запись отсановлена, файл сохарнён'")
    ...
    def handle_name_saved(self, data):
        if data['cancel']:
            self.status_label.setText('Запись отменена')
        else:
            #filename = str(Path(self.sounds_dir) / f"{data['filename']}.wav")
            filepath = os.path.join(self.sounds_dir, f"{data['file']}.wav")
            self.save_audio_file(str(filepath).replace('//', '/'))
            self.status_label.setText(f"Запсиь сохранена как {data['file']}")
